...
def plot_trajectories(csv_files):
    ...
    for idx, traj in enumerate(trajectories):
        ...
        # 使用中值滤波去噪（去除尖锐突变/毛刺点）
        # 窗口大小根据轨迹点数动态设置（保持奇数）
        window = max(3, len(traj) // 50)
        if window % 2 == 0:
            window += 1
            
        # 如果点数很少，限制最大窗口防止过度平滑
        window = min(window, 11) if len(traj) < 1000 else min(window, 51)
        
        smoothed = np.column_stack([
            medfilt(traj[:, i], kernel_size=window) for i in range(3)
        ])

        # 保护首尾点不被滤波扭曲（中值滤波边界效应）
        margin = window // 2
        if margin > 0:
            smoothed[:margin] = traj[:margin]
            smoothed[-margin:] = traj[-margin:]

        ax.plot(smoothed[:, 0], smoothed[:, 1], smoothed[:, 2],
