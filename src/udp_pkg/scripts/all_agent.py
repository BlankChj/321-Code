...
class Agent:
    def __init__(self, num, ids, server_host_list):
        ...
        self.cmd_pub = [rospy.Publisher(f"/iris{i}/information", PositionVelocityAccel, queue_size=10) for i in range(self.num - 1)]
                    ...
                    with port_locks[8100]:
                        for i in range(self.num - 1):
                            with port_locks[i]:
                                if attack_mode == 0.0:
                                    self.dos_flag = True
                                    self.cmd = copy.deepcopy(self.all_pose[i])
                                    self.history_pose[i].append(copy.deepcopy(self.cmd))
                                    self.cmd.frame_id = "world_0"
                                    self.cmd_pub[i].publish(self.cmd)
                                    ...
                                    self.dos_flag = True
                                    if len(self.history_pose[i]) > 0:
                                        self.cmd = copy.deepcopy(self.history_pose[i].popleft())
                                    else:
                                        self.cmd = copy.deepcopy(self.all_pose[i])
                                    self.history_pose[i].append(copy.deepcopy(self.all_pose[i]))
