...
class PosEstPlotter:
    def __init__(self, max_data_points=4000):
        ...
        # 初始化3D轨迹线 - 使用空numpy数组
        # 单元素元组解包需加,否则会被识别为括号
        self.x_line_normal, = self.ax1.plot([], [], 'b-', linewidth=2, alpha=0.7, label=' Normal Data')
        self.x_line_attack, = self.ax1.plot([], [], 'b-', linewidth=2, alpha=0.7, label=' Attack Data')
        self.x_line_kf, = self.ax1.plot([], [], 'b-', linewidth=2, alpha=0.7, label=' KF Data')
        self.x_line_rkf, = self.ax1.plot([], [], 'b-', linewidth=2, alpha=0.7, label=' RKF Data')

        self.y_line_normal, = self.ax2.plot([], [], 'b-', linewidth=2, alpha=0.7, label=' Normal Data')
        self.y_line_attack, = self.ax2.plot([], [], 'b-', linewidth=2, alpha=0.7, label=' Attack Data')
        self.y_line_kf, = self.ax2.plot([], [], 'b-', linewidth=2, alpha=0.7, label=' KF Data')
        self.y_line_rkf, = self.ax2.plot([], [], 'b-', linewidth=2, alpha=0.7, label=' RKF Data')

        self.z_line_normal, = self.ax3.plot([], [], 'b-', linewidth=2, alpha=0.7, label=' Normal Data')
        self.z_line_attack, = self.ax3.plot([], [], 'b-', linewidth=2, alpha=0.7, label=' Attack Data')
        self.z_line_kf, = self.ax3.plot([], [], 'b-', linewidth=2, alpha=0.7, label=' KF Data')
        self.z_line_rkf, = self.ax3.plot([], [], 'b-', linewidth=2, alpha=0.7, label=' RKF Data')
